...
- Source page: /docs/stable/clients/wasm/extensions.html
- Keywords: duckdb, docs, client apis, cli, node neo, rust, wasm, clients, wasm, extensions html
- Summary: [![Image 1: DuckDB Logo for Download](http://duckdb.org/images/logo-dl/DuckDB_Logo-horizontal.svg)](https://duckdb.org/)
----

Source: https://duckdb.org/docs/stable/clients/wasm/extensions.html
...
*   [1.5-dev"](http://duckdb.org/docs/preview/clients/wasm/extensions)
*   [1.4](http://duckdb.org/docs/stable/clients/wasm/extensions)
*   [1.3](http://duckdb.org/docs/1.3/clients/wasm/extensions)
*   [1.2](http://duckdb.org/docs/1.2/clients/wasm/extensions)
...
[Format](http://duckdb.org/docs/stable/clients/wasm/extensions.html#format)
---------------------------------------------------------------------------

Extensions in DuckDB are binaries to be dynamically loaded via 
```plaintext
dlopen
```
. A cryptographical signature is appended to the binary. An extension in DuckDB-Wasm is a regular Wasm file to be dynamically loaded via Emscripten's 
```plaintext
dlopen
```
. A cryptographical signature is appended to the Wasm file as a WebAssembly custom section called 
```plaintext
duckdb_signature
```
...
[```plaintext INSTALL ``` and ```plaintext LOAD ```](http://duckdb.org/docs/stable/clients/wasm/extensions.html#install-and-load)
---------------------------------------------------------------------------------------------------------------------------------
...
INSTALL
```
 semantic in native embeddings of DuckDB is to fetch, decompress from 
```plaintext
gzip
```
 and store data in local disk. The 
```plaintext
LOAD
```
 semantic in native embeddings of DuckDB is to (optionally) perform signature checks _and_ dynamic load the binary with the main DuckDB binary.

In DuckDB-Wasm, 
```plaintext
INSTALL
```
 is a no-op given there is no durable cross-session storage. The 
```plaintext
LOAD
```
 operation will fetch (and decompress on the fly), perform signature checks _and_ dynamically load via the Emscripten implementation of 
```plaintext
dlopen
```
.

[Autoloading](http://duckdb.org/docs/stable/clients/wasm/extensions.html#autoloading)
-------------------------------------------------------------------------------------

[Autoloading](http://duckdb.org/docs/stable/extensions/overview.html), i.e., the possibility for DuckDB to add extension functionality on-the-fly, is enabled by default in DuckDB-Wasm.

[List of Officially Available Extensions](http://duckdb.org/docs/stable/clients/wasm/extensions.html#list-of-officially-available-extensions)
---------------------------------------------------------------------------------------------------------------------------------------------

| Extension name | Description | Aliases |
| --- | --- | --- |
| [autocomplete](http://duckdb.org/docs/stable/core_extensions/autocomplete.html) | Adds support for autocomplete in the shell |  |
| [excel](http://duckdb.org/docs/stable/core_extensions/excel.html) | Adds support for Excel-like format strings |  |
| [fts](http://duckdb.org/docs/stable/core_extensions/full_text_search.html) | Adds support for Full-Text Search Indexes |  |
| [icu](http://duckdb.org/docs/stable/core_extensions/icu.html) | Adds support for time zones and collations using the ICU library |  |
| [inet](http://duckdb.org/docs/stable/core_extensions/inet.html) | Adds support for IP-related data types and functions |  |
| [json](http://duckdb.org/docs/stable/data/json/overview.html) | Adds support for JSON operations |  |
| [parquet](http://duckdb.org/docs/stable/data/parquet/overview.html) | Adds support for reading and writing Parquet files |  |
| [sqlite](http://duckdb.org/docs/stable/core_extensions/sqlite.html) | Adds support for reading SQLite database files | sqlite, sqlite3 |
| [sqlsmith](http://duckdb.org/docs/stable/core_extensions/sqlsmith.html) |  |  |
| [tpcds](http://duckdb.org/docs/stable/core_extensions/tpcds.html) | Adds TPC-DS data generation and query support |  |
| [tpch](http://duckdb.org/docs/stable/core_extensions/tpch.html) | Adds TPC-H data generation and query support |  |
...
### [HTTPFS](http://duckdb.org/docs/stable/clients/wasm/extensions.html#httpfs)
...
[Extension Signing](http://duckdb.org/docs/stable/clients/wasm/extensions.html#extension-signing)
-------------------------------------------------------------------------------------------------

As with regular DuckDB extensions, DuckDB-Wasm extension are by default checked on 
```plaintext
LOAD
```
...
[Fetching DuckDB-Wasm Extensions](http://duckdb.org/docs/stable/clients/wasm/extensions.html#fetching-duckdb-wasm-extensions)
-----------------------------------------------------------------------------------------------------------------------------

Official DuckDB extensions are served at 
```plaintext
extensions.duckdb.org
```
, and this is also the default value for the 
```plaintext
default_extension_repository
```
 option. When installing extensions, a relevant URL will be built that will look like 
```plaintext
extensions.duckdb.org/$duckdb_version_hash/$duckdb_platform/$name.duckdb_extension.gz
```
.

DuckDB-Wasm extension are fetched only on load, and the URL will look like: 
```plaintext
...
duckdb-wasm
```
 is added to the folder structure, and the file is served as a 
```plaintext
.wasm
```
 file.

DuckDB-Wasm extensions are served pre-compressed using Brotli compression. While fetched from a browser, extensions will be transparently uncompressed. If you want to fetch the 
```plaintext
duckdb-wasm
```
...
[Serving Extensions from a Third-Party Repository](http://duckdb.org/docs/stable/clients/wasm/extensions.html#serving-extensions-from-a-third-party-repository)
---------------------------------------------------------------------------------------------------------------------------------------------------------------

As with regular DuckDB, if you use 
```plaintext
SET custom_extension_repository = 'https://some.url.com'
```
, subsequent loads will be attempted at 
```plaintext
https://some.url.com/duckdb-wasm/$duckdb_version_hash/$duckdb_platform/$name.duckdb_extension.wasm
```
.
...
[Tooling](http://duckdb.org/docs/stable/clients/wasm/extensions.html#tooling)
-----------------------------------------------------------------------------
...
*   [Report content issue](https://github.com/duckdb/duckdb-web/issues/new?title=Issue%20found%20on%20page%20%27Extensions%27&labels=issue%20found%20on%20page&body=%0A%3E%20Please%20describe%20the%20problem%20you%20encountered%20in%20the%20DuckDB%20documentation%20and%20include%20the%20%22Page%20URL%22%20link%20shown%20below.%0A%3E%20Note:%20only%20create%20an%20issue%20if%20you%20wish%20to%20report%20a%20problem%20with%20the%20DuckDB%20documentation.%20For%20questions%20about%20DuckDB%20or%20the%20use%20of%20certain%20DuckDB%20features,%20use%20[GitHub%20Discussions](https://github.com/duckdb/duckdb/discussions/),%20[Stack%20Overflow](https://stackoverflow.com/questions/tagged/duckdb),%20or%20[Discord](https://discord.duckdb.org/).%0A%0APage%20URL:%20%3Chttps://duckdb.org/docs/stable/clients/wasm/extensions.html%3E%0A "Create GitHub issue")
*   [See this page as Markdown](https://raw.githubusercontent.com/duckdb/duckdb-web/refs/heads/main/docs/stable/clients/wasm/extensions.md "See Markdown")
*   [Edit this page on GitHub](https://github.com/duckdb/duckdb-web/edit/main/docs/stable/clients/wasm/extensions.md "Go to GitHub")
...
*   [Format](http://duckdb.org/docs/stable/clients/wasm/extensions.html#format)
*   [INSTALL and LOAD](http://duckdb.org/docs/stable/clients/wasm/extensions.html#install-and-load)
*   [Autoloading](http://duckdb.org/docs/stable/clients/wasm/extensions.html#autoloading)
*   [List of Officially Available Extensions](http://duckdb.org/docs/stable/clients/wasm/extensions.html#list-of-officially-available-extensions)
    *   [HTTPFS](http://duckdb.org/docs/stable/clients/wasm/extensions.html#httpfs)
