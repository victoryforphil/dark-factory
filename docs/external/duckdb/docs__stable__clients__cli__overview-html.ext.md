...
- Source page: /docs/stable/clients/cli/overview.html
- Keywords: duckdb, docs, client apis, cli, node neo, rust, wasm, clients, cli, overview html
- Summary: > The latest stable version of the DuckDB command line client is 1.4.4.
----

Source: https://duckdb.org/docs/stable/clients/cli/overview.html

Command Line Client

> The latest stable version of the DuckDB command line client is 1.4.4.

[Installation](http://duckdb.org/docs/stable/clients/cli/overview.html#installation)
------------------------------------------------------------------------------------
...
The DuckDB CLI is based on the SQLite command line shell, so CLI-client-specific functionality is similar to what is described in the [SQLite documentation](https://www.sqlite.org/cli.html) (although DuckDB's SQL syntax follows PostgreSQL conventions with a [few exceptions](https://duckdb.org/docs/stable/sql/dialect/postgresql_compatibility.html)).

> DuckDB has a [tldr page](https://tldr.inbrowser.app/pages/common/duckdb), which summarizes the most common uses of the CLI client. If you have [tldr](https://github.com/tldr-pages/tldr) installed, you can display it by running `tldr duckdb`.

[Getting Started](http://duckdb.org/docs/stable/clients/cli/overview.html#getting-started)
------------------------------------------------------------------------------------------
...
[Usage](http://duckdb.org/docs/stable/clients/cli/overview.html#usage)
----------------------------------------------------------------------
...
### [Options](http://duckdb.org/docs/stable/clients/cli/overview.html#options)

The `OPTIONS` part encodes [arguments for the CLI client](https://duckdb.org/docs/stable/clients/cli/arguments.html). Common options include:

*   `-csv`: sets the output mode to CSV
*   `-json`: sets the output mode to JSON
*   `-readonly`: open the database in read-only mode (see [concurrency in DuckDB](https://duckdb.org/docs/stable/connect/concurrency.html#handling-concurrency))

For a full list of options, see the [command line arguments page](https://duckdb.org/docs/stable/clients/cli/arguments.html).

### [In-Memory vs. Persistent Database](http://duckdb.org/docs/stable/clients/cli/overview.html#in-memory-vs-persistent-database)

When no `FILENAME` argument is provided, the DuckDB CLI will open a temporary [in-memory database](https://duckdb.org/docs/stable/connect/overview.html#in-memory-database). You will see DuckDB's version number, the information on the connection and a prompt starting with a `D`.
...
To open or create a [persistent database](https://duckdb.org/docs/stable/connect/overview.html#persistent-database), simply include a path as a command line argument:

```
duckdb my_database.duckdb
```

### [Running SQL Statements in the CLI](http://duckdb.org/docs/stable/clients/cli/overview.html#running-sql-statements-in-the-cli)
...
The CLI supports all of DuckDB's rich [SQL syntax](https://duckdb.org/docs/stable/sql/introduction.html) including `SELECT`, `CREATE` and `ALTER` statements.

### [Editor Features](http://duckdb.org/docs/stable/clients/cli/overview.html#editor-features)

The CLI supports [autocompletion](https://duckdb.org/docs/stable/clients/cli/autocomplete.html), and has sophisticated [editor features](https://duckdb.org/docs/stable/clients/cli/editing.html) and [syntax highlighting](https://duckdb.org/docs/stable/clients/cli/syntax_highlighting.html) on certain platforms.

### [Exiting the CLI](http://duckdb.org/docs/stable/clients/cli/overview.html#exiting-the-cli)
...
### [Dot Commands](http://duckdb.org/docs/stable/clients/cli/overview.html#dot-commands)

In addition to SQL syntax, special [dot commands](https://duckdb.org/docs/stable/clients/cli/dot_commands.html) may be entered into the CLI client. To use one of these commands, begin the line with a period (`.`) immediately followed by the name of the command you wish to execute. Additional arguments to the command are entered, space separated, after the command. If an argument must contain a space, either single or double quotes may be used to wrap that parameter. Dot commands must be entered on a single line, and no whitespace may occur before the period. No semicolon is required at the end of the line.

Frequently-used configurations can be stored in the file `~/.duckdbrc`, which will be loaded when starting the CLI client. See the [Configuring the CLI](http://duckdb.org/docs/stable/clients/cli/overview.html#configuring-the-cli) section below for further information on these options.
...
Below, we summarize a few important dot commands. To see all available commands, see the [dot commands page](https://duckdb.org/docs/stable/clients/cli/dot_commands.html) or use the `.help` command.

#### [Opening Database Files](http://duckdb.org/docs/stable/clients/cli/overview.html#opening-database-files)
...
> 
> 
> `.open` closes the current database. To keep the current database, while adding a new database, use the [`ATTACH` statement](https://duckdb.org/docs/stable/sql/statements/attach.html).
...
#### [Output Formats](http://duckdb.org/docs/stable/clients/cli/overview.html#output-formats)

The `.mode`[dot command](https://duckdb.org/docs/stable/clients/cli/dot_commands.html#mode) may be used to change the appearance of the tables returned in the terminal output. These include the default `duckbox` mode, `csv` and `json` mode for ingestion by other tools, `markdown` and `latex` for documents and `insert` mode for generating SQL statements.

#### [Writing Results to a File](http://duckdb.org/docs/stable/clients/cli/overview.html#writing-results-to-a-file)

By default, the DuckDB CLI sends results to the terminal's standard output. However, this can be modified using either the `.output` or `.once` commands. For details, see the documentation for the [output dot command](https://duckdb.org/docs/stable/clients/cli/dot_commands.html#output-writing-results-to-a-file).

#### [Reading SQL from a File](http://duckdb.org/docs/stable/clients/cli/overview.html#reading-sql-from-a-file)
...
[Configuring the CLI](http://duckdb.org/docs/stable/clients/cli/overview.html#configuring-the-cli)
--------------------------------------------------------------------------------------------------
...
### [Setting a Custom Prompt](http://duckdb.org/docs/stable/clients/cli/overview.html#setting-a-custom-prompt)
...
[Non-Interactive Usage](http://duckdb.org/docs/stable/clients/cli/overview.html#non-interactive-usage)
------------------------------------------------------------------------------------------------------
...
[Loading Extensions](http://duckdb.org/docs/stable/clients/cli/overview.html#loading-extensions)
------------------------------------------------------------------------------------------------
...
For details, see the [Extension docs](https://duckdb.org/docs/stable/extensions/overview.html).

[Reading from stdin and Writing to stdout](http://duckdb.org/docs/stable/clients/cli/overview.html#reading-from-stdin-and-writing-to-stdout)
--------------------------------------------------------------------------------------------------------------------------------------------
...
[Reading Environment Variables](http://duckdb.org/docs/stable/clients/cli/overview.html#reading-environment-variables)
----------------------------------------------------------------------------------------------------------------------

The `getenv` function can read environment variables.

### [Examples](http://duckdb.org/docs/stable/clients/cli/overview.html#examples)
...
The output of the `getenv` function can be used to set [configuration options](https://duckdb.org/docs/stable/configuration/overview.html). For example, to set the `NULL` order based on the environment variable `DEFAULT_NULL_ORDER`, use:

```
SET default_null_order = getenv('DEFAULT_NULL_ORDER');
```

### [Restrictions for Reading Environment Variables](http://duckdb.org/docs/stable/clients/cli/overview.html#restrictions-for-reading-environment-variables)

The `getenv` function can only be run when the [`enable_external_access`](https://duckdb.org/docs/stable/configuration/overview.html#configuration-reference) option is set to `true` (the default setting). It is only available in the CLI client and is not supported in other DuckDB clients.

[Prepared Statements](http://duckdb.org/docs/stable/clients/cli/overview.html#prepared-statements)
--------------------------------------------------------------------------------------------------

The DuckDB CLI supports executing [prepared statements](https://duckdb.org/docs/stable/sql/query_syntax/prepared_statements.html) in addition to regular `SELECT` statements. To create and execute a prepared statement in the CLI client, use the `PREPARE` clause and the `EXECUTE` statement.

[Query Completion ETA](http://duckdb.org/docs/stable/clients/cli/overview.html#query-completion-eta)
----------------------------------------------------------------------------------------------------
...
### [How It Works](http://duckdb.org/docs/stable/clients/cli/overview.html#how-it-works)
...
### [Factors Affecting The Accuracy of Query Completion ETA](http://duckdb.org/docs/stable/clients/cli/overview.html#factors-affecting-the-accuracy-of-query-completion-eta)
