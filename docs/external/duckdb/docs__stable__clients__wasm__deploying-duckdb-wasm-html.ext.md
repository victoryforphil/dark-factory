...
- Source page: /docs/stable/clients/wasm/deploying_duckdb_wasm.html
- Keywords: duckdb, docs, client apis, cli, node neo, rust, wasm, clients, wasm, deploying duckdb wasm html
- Summary: Deploying DuckDB-Wasm – DuckDB
----

Source: https://duckdb.org/docs/stable/clients/wasm/deploying_duckdb_wasm.html

Deploying DuckDB-Wasm – DuckDB
===============
...
*   [1.5-dev"](http://duckdb.org/docs/preview/clients/wasm/deploying_duckdb_wasm)
*   [1.4](http://duckdb.org/docs/stable/clients/wasm/deploying_duckdb_wasm)
*   [1.3](http://duckdb.org/docs/1.3/clients/wasm/deploying_duckdb_wasm)
...
[Main Library Component](http://duckdb.org/docs/stable/clients/wasm/deploying_duckdb_wasm.html#main-library-component)
----------------------------------------------------------------------------------------------------------------------

This is distributed as either TypeScript code or CommonJS JavaScript code in the 
```plaintext
npm
```
 duckdb-wasm package, and can be either bundled together with a given application, served in a same origin (sub-)domain and included at runtime or served from a third party CDN like JSDelivery. This does need some form of transpilation and can't be served as-is, given it needs to know the location of the follow up files for this to be functional. Details will depend on your given setup, examples can be found at [https://github.com/duckdb/duckdb-wasm/tree/main/examples](https://github.com/duckdb/duckdb-wasm/tree/main/examples). An example deployment is [https://shell.duckdb.org](https://shell.duckdb.org/), which transpiles the main library component together with shell code (first approach). Or the 
```plaintext
bare-browser
```
 example at [https://github.com/duckdb/duckdb-wasm/tree/main/examples/bare-browser](https://github.com/duckdb/duckdb-wasm/tree/main/examples/bare-browser).

[JS Worker Component](http://duckdb.org/docs/stable/clients/wasm/deploying_duckdb_wasm.html#js-worker-component)
----------------------------------------------------------------------------------------------------------------

This is distributed as a JavaScript file in 3 different flavors, 
```plaintext
mvp
```
, 
...
threads
```
, and needs to be served as is. The main library components need to be informed of the actual location.

There are 3 variants for 3 different 
```plaintext
platforms
```
:
...
mvp
```
 targets WebAssembly 1.0 spec
*   ```plaintext
eh
```
 targets WebAssembly 1.0 spec WITH Wasm-level exceptions handling added, which improves performance
*   ```plaintext
threads
```
 targets WebAssembly spec WITH exception and threading constructs

You could serve all 3, and feature detect, or serve a single variant and instruct duckdb-wasm library on which one to use

[Wasm Worker Component](http://duckdb.org/docs/stable/clients/wasm/deploying_duckdb_wasm.html#wasm-worker-component)
--------------------------------------------------------------------------------------------------------------------

Same as the JS Worker component, 3 different flavors, 
```plaintext
mvp
```
, 
...
threads
```
, each one is needed by the relevant JS component. These WebAssembly modules needs to be served as-is at an arbitrary [sub-] domain that is reachable from the main one.

[DuckDB Extensions](http://duckdb.org/docs/stable/clients/wasm/deploying_duckdb_wasm.html#duckdb-extensions)
------------------------------------------------------------------------------------------------------------

DuckDB extensions for DuckDB-Wasm, similar for the native cases, are served signed at the default extension endpoint: 
```plaintext
...
```plaintext
https://extensons.duckdb.org
```
 to the one specified. Note that extensions are still signed, so the best path is downloading and serving the extensions with a similar structure to the original repository. See additional notes at [https://duckdb.org/docs/stable/extensions/extension_distribution#creating-a-custom-repository](https://duckdb.org/docs/stable/extensions/extension_distribution#creating-a-custom-repository).

Community extensions are served at [https://community-extensioions.duckdb.org](https://community-extensioions.duckdb.org/), and they are signed with a different key, so they can be disabled with a one way SQL statement such as:

```
SET allow_community_extensions = false;
```

This will allow loading **only** of core duckdb extensions. Note that the failure is at 
```plaintext
LOAD
```
 time, not at 
...
Please review [https://duckdb.org/docs/stable/extensions/extension_distribution](https://duckdb.org/docs/stable/extensions/extension_distribution) for general information about extensions.

[Security Considerations](http://duckdb.org/docs/stable/clients/wasm/deploying_duckdb_wasm.html#security-considerations)
------------------------------------------------------------------------------------------------------------------------
...
*   [Report content issue](https://github.com/duckdb/duckdb-web/issues/new?title=Issue%20found%20on%20page%20%27Deploying%20DuckDB-Wasm%27&labels=issue%20found%20on%20page&body=%0A%3E%20Please%20describe%20the%20problem%20you%20encountered%20in%20the%20DuckDB%20documentation%20and%20include%20the%20%22Page%20URL%22%20link%20shown%20below.%0A%3E%20Note:%20only%20create%20an%20issue%20if%20you%20wish%20to%20report%20a%20problem%20with%20the%20DuckDB%20documentation.%20For%20questions%20about%20DuckDB%20or%20the%20use%20of%20certain%20DuckDB%20features,%20use%20[GitHub%20Discussions](https://github.com/duckdb/duckdb/discussions/),%20[Stack%20Overflow](https://stackoverflow.com/questions/tagged/duckdb),%20or%20[Discord](https://discord.duckdb.org/).%0A%0APage%20URL:%20%3Chttps://duckdb.org/docs/stable/clients/wasm/deploying_duckdb_wasm.html%3E%0A "Create GitHub issue")
*   [See this page as Markdown](https://raw.githubusercontent.com/duckdb/duckdb-web/refs/heads/main/docs/stable/clients/wasm/deploying_duckdb_wasm.md "See Markdown")
*   [Edit this page on GitHub](https://github.com/duckdb/duckdb-web/edit/main/docs/stable/clients/wasm/deploying_duckdb_wasm.md "Go to GitHub")
