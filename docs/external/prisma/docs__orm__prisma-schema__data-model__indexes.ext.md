...
- Source page: /docs/orm/prisma-schema/data-model/indexes
- Keywords: prisma, prisma schema, orm, docs, docs, orm, prisma schema, data model, indexes
- Summary: Prisma ORM allows configuration of database indexes, unique constraints and primary key constraints. This is in General Availability in versions `4.0.0` and later. You can enable this with the `extendedIndexes` Preview feature in version...
----
...
If you are upgrading from a version earlier than 4.0.0, these changes to index configuration and full text indexes might be **breaking changes** if you have a database that already uses these features. See [Upgrading from previous versions](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#upgrading-from-previous-versions) for more information on how to upgrade.

Index configuration[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#index-configuration "Direct link to Index configuration")
-----------------------------------------------------------------------------------------------------------------------------------------------

You can configure indexes, unique constraints, and primary key constraints with the following attribute arguments:

*   The [`length` argument](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#configuring-the-length-of-indexes-with-length-mysql) allows you to specify a maximum length for the subpart of the value to be indexed on `String` and `Bytes` types

    *   Available on the `@id`, `@@id`, `@unique`, `@@unique` and `@@index` attributes
    *   MySQL only

*   The [`sort` argument](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#configuring-the-index-sort-order-with-sort) allows you to specify the order that the entries of the constraint or index are stored in the database

    *   Available on the `@unique`, `@@unique` and `@@index` attributes in all databases, and on the `@id` and `@@id` attributes in SQL Server

*   The [`type` argument](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#configuring-the-access-type-of-indexes-with-type-postgresql) allows you to support index access methods other than PostgreSQL's default `BTree` access method

    *   Available on the `@@index` attribute
    *   PostgreSQL only
    *   Supported index access methods: `Hash`, `Gist`, `Gin`, `SpGist` and `Brin`

*   The [`clustered` argument](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#configuring-if-indexes-are-clustered-or-non-clustered-with-clustered-sql-server) allows you to configure whether a constraint or index is clustered or non-clustered

    *   Available on the `@id`, `@@id`, `@unique`, `@@unique` and `@@index` attributes
    *   SQL Server only

*   The [`map` argument](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#configuring-the-name-of-indexes-with-map) allows you to specify a custom name for the index or constraint in the underlying database
...
### Configuring the length of indexes with `length` (MySQL)[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#configuring-the-length-of-indexes-with-length-mysql "Direct link to configuring-the-length-of-indexes-with-length-mysql")
...
### Configuring the index sort order with `sort`[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#configuring-the-index-sort-order-with-sort "Direct link to configuring-the-index-sort-order-with-sort")
...
`-- PostgreSQL approachCREATE UNIQUE INDEX "unique_index_desc" ON "Unique" ("unique" DESC);`
...
### Example: using `sort` and `length` together[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#example-using-sort-and-length-together "Direct link to example-using-sort-and-length-together")
...
### Configuring the access type of indexes with `type` (PostgreSQL)[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#configuring-the-access-type-of-indexes-with-type-postgresql "Direct link to configuring-the-access-type-of-indexes-with-type-postgresql")
...
#### Hash[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#hash "Direct link to Hash")
...
#### Generalized Inverted Index (GIN)[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#generalized-inverted-index-gin "Direct link to Generalized Inverted Index (GIN)")
...
`model Example {  id    Int  @id  value Json  //    ^ field type matching the operator class  @@index([value(ops: JsonbPathOps)], type: Gin)  //                  ^ operator class      ^ index type}`
...
##### Supported Operator Classes for GIN[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#supported-operator-classes-for-gin "Direct link to Supported Operator Classes for GIN")
...
##### CockroachDB[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#cockroachdb "Direct link to CockroachDB")
...
#### Generalized Search Tree (GiST)[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#generalized-search-tree-gist "Direct link to Generalized Search Tree (GiST)")
...
##### Supported Operator Classes for GiST[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#supported-operator-classes-for-gist "Direct link to Supported Operator Classes for GiST")
...
#### Space-Partitioned GiST (SP-GiST)[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#space-partitioned-gist-sp-gist "Direct link to Space-Partitioned GiST (SP-GiST)")
...
##### Supported Operator Classes for SP-GiST[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#supported-operator-classes-for-sp-gist "Direct link to Supported Operator Classes for SP-GiST")
...
#### Block Range Index (BRIN)[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#block-range-index-brin "Direct link to Block Range Index (BRIN)")
...
`model Example {  id    Int @id  value Int  //    ^ field type matching the operator class  @@index([value(ops: Int4BloomOps)], type: Brin)  //                  ^ operator class      ^ index type}`
...
##### Supported Operator Classes for BRIN[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#supported-operator-classes-for-brin "Direct link to Supported Operator Classes for BRIN")
...
### Configuring if indexes are clustered or non-clustered with `clustered` (SQL Server)[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#configuring-if-indexes-are-clustered-or-non-clustered-with-clustered-sql-server "Direct link to configuring-if-indexes-are-clustered-or-non-clustered-with-clustered-sql-server")
...
### Configuring the name of indexes with `map`[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#configuring-the-name-of-indexes-with-map "Direct link to configuring-the-name-of-indexes-with-map")
...
`model Post {  id    Int    @id  title String  @@index([title], map: "my_custom_index_name")}`
...
`model User {  id    Int    @id  email String @unique(map: "unique_user_email")}`

And on composite indexes and constraints:

schema.prisma

`model Post {  id        Int    @id  title     String  author    String  createdAt DateTime  @@index([author, createdAt], map: "posts_author_date_idx")  @@unique([title, author], map: "posts_title_author_unique")}`

### Upgrading from previous versions[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#upgrading-from-previous-versions "Direct link to Upgrading from previous versions")
...
Full text indexes (MySQL and MongoDB)[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#full-text-indexes-mysql-and-mongodb "Direct link to Full text indexes (MySQL and MongoDB)")
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
### Enabling the `fullTextIndex` preview feature[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#enabling-the-fulltextindex-preview-feature "Direct link to enabling-the-fulltextindex-preview-feature")
...
`generator client {  provider        = "prisma-client"  output          = "./generated"  previewFeatures = ["fullTextIndex"]}`

### Examples[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#examples "Direct link to Examples")
...
### Upgrading from previous versions[​](http://www.prisma.io/docs/orm/prisma-schema/data-model/indexes#upgrading-from-previous-versions-1 "Direct link to Upgrading from previous versions")
