----
## External Docs Index // opencode

- Captured: 2026-02-16T07:29:35.337Z
- Source root: https://opencode.ai/docs
- Output directory: docs/external/opencode
- Discovery: sitemap (sitemap_url=https://opencode.ai/sitemap.xml,matched=34,lang=en)
- Scope: 34 pages under /docs
- Keywords: opencode, docs index, ai coding assistant, cli
- Summary: This index links one `.ext.md` file per docs page snapshot for OpenCode.
----

## Pages

- [docs.ext.md](./docs.ext.md) - /docs (ok,mode=jina)
- [docs__acp.ext.md](./docs__acp.ext.md) - /docs/acp (ok,mode=jina)
- [docs__agents.ext.md](./docs__agents.ext.md) - /docs/agents (ok,mode=jina)
- [docs__cli.ext.md](./docs__cli.ext.md) - /docs/cli (ok,mode=jina)
- [docs__commands.ext.md](./docs__commands.ext.md) - /docs/commands (ok,mode=jina)
- [docs__config.ext.md](./docs__config.ext.md) - /docs/config (ok,mode=jina)
- [docs__custom-tools.ext.md](./docs__custom-tools.ext.md) - /docs/custom-tools (ok,mode=jina)
- [docs__ecosystem.ext.md](./docs__ecosystem.ext.md) - /docs/ecosystem (ok,mode=jina)
- [docs__enterprise.ext.md](./docs__enterprise.ext.md) - /docs/enterprise (ok,mode=jina)
- [docs__formatters.ext.md](./docs__formatters.ext.md) - /docs/formatters (ok,mode=jina)
- [docs__github.ext.md](./docs__github.ext.md) - /docs/github (ok,mode=jina)
- [docs__gitlab.ext.md](./docs__gitlab.ext.md) - /docs/gitlab (ok,mode=jina)
- [docs__ide.ext.md](./docs__ide.ext.md) - /docs/ide (ok,mode=jina)
- [docs__keybinds.ext.md](./docs__keybinds.ext.md) - /docs/keybinds (ok,mode=jina)
- [docs__lsp.ext.md](./docs__lsp.ext.md) - /docs/lsp (ok,mode=jina)
- [docs__mcp-servers.ext.md](./docs__mcp-servers.ext.md) - /docs/mcp-servers (ok,mode=jina)
- [docs__models.ext.md](./docs__models.ext.md) - /docs/models (ok,mode=jina)
- [docs__modes.ext.md](./docs__modes.ext.md) - /docs/modes (ok,mode=jina)
- [docs__network.ext.md](./docs__network.ext.md) - /docs/network (ok,mode=jina)
- [docs__permissions.ext.md](./docs__permissions.ext.md) - /docs/permissions (ok,mode=jina)
- [docs__plugins.ext.md](./docs__plugins.ext.md) - /docs/plugins (ok,mode=jina)
- [docs__providers.ext.md](./docs__providers.ext.md) - /docs/providers (ok,mode=html)
- [docs__rules.ext.md](./docs__rules.ext.md) - /docs/rules (ok,mode=html)
- [docs__sdk.ext.md](./docs__sdk.ext.md) - /docs/sdk (ok,mode=html)
- [docs__server.ext.md](./docs__server.ext.md) - /docs/server (ok,mode=html)
- [docs__share.ext.md](./docs__share.ext.md) - /docs/share (ok,mode=html)
- [docs__skills.ext.md](./docs__skills.ext.md) - /docs/skills (ok,mode=html)
- [docs__themes.ext.md](./docs__themes.ext.md) - /docs/themes (ok,mode=html)
- [docs__tools.ext.md](./docs__tools.ext.md) - /docs/tools (ok,mode=html)
- [docs__troubleshooting.ext.md](./docs__troubleshooting.ext.md) - /docs/troubleshooting (ok,mode=html)
- [docs__tui.ext.md](./docs__tui.ext.md) - /docs/tui (ok,mode=html)
- [docs__web.ext.md](./docs__web.ext.md) - /docs/web (ok,mode=html)
- [docs__windows-wsl.ext.md](./docs__windows-wsl.ext.md) - /docs/windows-wsl (ok,mode=html)
- [docs__zen.ext.md](./docs__zen.ext.md) - /docs/zen (ok,mode=html)

----
## Notes / Comments / Lessons

- Per-page files are flattened in this directory and prefixed with `docs` in the filename stem.
- Regenerate by re-running the scraper script; old `.ext.md` files in this directory are replaced.
- Capture results: success=34, failed=0.
----
