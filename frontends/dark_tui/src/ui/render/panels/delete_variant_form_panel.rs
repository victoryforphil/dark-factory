...
impl DeleteVariantFormPanel {
    pub(crate) fn render(frame: &mut Frame, area: Rect, app: &App) {
        ...
        let behavior = if remove_clone_dir {
            "Delete row + remove clone directory (dry=false)"
        } else {
            "Delete row only, keep clone directory (dry=true)"
        };
