...
function fileStemFromUrl(url: string): string {
  ...
  if (sourcePath === ROOT_PATH) {
    return "docs__orm__prisma-schema";
  }

  const relative = sourcePath.replace(/^\/docs\/?/, "");
  const segments = relative.split("/").filter(Boolean).map(sanitizeSegment);
...
function extractSummary(markdown: string): string {
  ...
  return "Prisma ORM prisma-schema documentation page snapshot.";
}
...
function keywordsForPath(sourcePath: string): string {
  ...
  const base = "prisma, prisma schema, orm, docs";
...
function buildIndexMarkdown(
  ...
  return [
    "----",
    "## External Docs Index // prisma",
    "",
    `- Captured: ${capturedAt}`,
    `- Source root: ${DOCS_ROOT}`,
    `- Output directory: ${outputDirRelative}`,
    `- Discovery: ${discovery.method} (${discovery.detail})`,
    `- Scope: ${artifacts.length} pages under ${ROOT_PATH}`,
    "- Keywords: prisma, prisma schema, docs index, orm",
    "- Summary: This index links one `.ext.md` file per docs page snapshot for Prisma schema docs.",
    "----",
    "",
    "## Pages",
    "",
...
async function discoverDocsUrlsFromRootPage(rootUrl: string): Promise<string[]> {
  try {
    const html = await fetchText(rootUrl);
    const linkedUrls = extractHrefUrls(html).map((url) => normalizeUrl(url)).filter((url): url is string => Boolean(url));
...
async function discoverDocsUrls(): Promise<DiscoveryResult> {
  ...
  const htmlUrls = await discoverDocsUrlsFromRootPage(DOCS_ROOT);
  return {
    urls: htmlUrls,
    method: "html",
